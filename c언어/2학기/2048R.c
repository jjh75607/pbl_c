...
#include <termios.h> //conio대체
#include <unistd.h> // conio대체
#include <stdlib.h> // windows.h 대체

//방향키 입력 이지만 윈도우와 맥의 아스키값이 다르게 들어감. 이유 모름
#define LEFT 68
#define RIGHT 67
#define UP 65
#define DOWN 66
...
int main(){
    ...
    int act = 0; // key입력후 게임판에 변화가 있었는지 저장
    ...
    while(1){
        ...
        if( (key == 27 || key == 91) || key == 0){ //방향키의 경우 키입력시 0xE0+키값이 두개 전달됨
